...
namespace LogApplication
{
     ...
     * 使用方法：
     * 在Global.asax的Application_Start方法中调用： Utility.LogServiceHelper.Instance.Start();
     * 在使用的地方调用 LogServiceHelper.Instance.PutLog(new LogModel() { CreatedTime=DateTime.Now, Dir="operator", Msg=log.URL, Operator=log.LoginName });
     * 在关闭的地方调用LogServiceHelper.Instance.Stop();
    ...
    public class LogServiceHelper
    {
        ...
        public static readonly LogServiceHelper Instance = new();
        ...
        private readonly int _maxWriteErrorTime = 10; //最大连续写入错误次数
        private int _currentWriteErrorTime; //当前连续写入错误次数
        private string _baseDirectory = string.Empty;
        private LogModel _tmpLog;

        private readonly ConcurrentQueue<LogModel> queue = new(); //日志队列      

        //对不同类型的日志进行区分合并
        private readonly List<LogBatch> _logBatch = new(); //批处理列表
        private LogBatch _tmpLogBatch;
        ...
        public void Start(string baseDirectory, int maxByteSize = 65536, int maxFileSize = 1048576, int maxSecond = 30)
        {
            ...
            _thread = new Thread(TestExecuting);
        ...
        private void TestExecuting()
        {
            _isStart = true;
            while (_isStart)
            {
                if (queue.Count > 0) QueueOut();
        ...
        private void QueueOut()
        {
            ...
            if (_tmpLog != null)
            {
                ...
                _tmpLogBatch.LogStr = _tmpLogBatch.LogStr.AppendFormat("时间【{0}】，操作人【{1}】，信息【{2}】" + Environment.NewLine,
                    _tmpLog.CreatedTime.ToString("yyyy-MM-dd HH:mm ss"), _tmpLog.Operator, _tmpLog.Msg);
                _tmpLog = null;
            }
        }
        ...
        private void LogBatchWrite()
        {
            ...
            {
                var second = (DateTime.Now - item.StartTime).Seconds;
        ...
        ///     日志写入
        /// </summary>
        /// <param name="logType">日志类型，如登录日志(log)，错误日志(error)，操作日志(operator)</param>
        ...
        private bool WriteFile(string log, string sDirectory)
        {
            ...
            var dir = Path.Combine(_baseDirectory, year, month);
            try
            {
                ...
                else
                {
                    ...
                    var filename = sDirectory + "_" + now.ToString("yyyy年MM月dd日") + "(" + index + ").log";
                    var fileNameNew = Path.Combine(dir, filename); //文件不存在则创建
            ...
            catch (Exception ex)
            {
                ...
                if (_currentWriteErrorTime > _maxWriteErrorTime) Stop(); //停止写入日志，发送报警短信
